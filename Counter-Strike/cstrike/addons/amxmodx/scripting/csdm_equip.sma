...
public m_EquipHandler(id, menu, item)
{
	...
	switch(choice)
	{
		...
		case 3:
		{
			...
			client_color(id, id, "^x04【5ePlay.com】^x01你选择了记住设置，重新打开武器菜单请^x03 say guns ^x01.");
			equipUser(id, EQUIP_ALL)
		}
	}
...
{
	//Equip Menu
	menu_additem(g_EquipMenuID, "新的武器", "1", 0, -1)
	menu_additem(g_EquipMenuID, "上一次武器", "2", 0, -1)
	menu_additem(g_EquipMenuID, "记住我的选项", "3", 0, -1)
	menu_setprop(g_EquipMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Armor Menu
	menu_additem(g_ArmorMenuID, "需要装甲", "1", 0, -1)
	menu_additem(g_ArmorMenuID, "不需要", "2", 0, -1)
	menu_setprop(g_ArmorMenuID, MPROP_EXIT, MEXIT_NEVER)
	
	//Nade Menu
	menu_additem(g_NadeMenuID, "全部手雷", "1", 0, -1)
	menu_additem(g_NadeMenuID, "不需要", "2", 0, -1)
...
public enableMenu(id)
{
	...
	{
		g_mShowuser[id] = true
		client_color(id, id, "^x04【5ePlay.com】^x01已经重新启用了你的装备菜单.");
		if (!g_MenuState[id])
		{
			g_MenuState[id] = 1
		...
		client_color(id, id, "^x04【5ePlay.com】^x01你的装备菜单已被启用.");
	}
	
	return PLUGIN_HANDLED
}

stock client_color(playerid, colorid, msg[], {Float,Sql,Result,_}:...)
{
	new text[256];
	vformat(text, 255, msg, 4);
	message_begin(playerid ? MSG_ONE : MSG_ALL, g_iMsgSayText, {0,0,0}, playerid);
	write_byte(colorid);
	write_string(text);
	message_end();
}
